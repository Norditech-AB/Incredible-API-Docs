...
# 💾 Store this file.id in your database!
...
db.save(user_id, file.id)  # Store in database

# Use file_id many times
response1 = client.answer(query="Q1?", file_ids=[file.id])
response2 = client.answer(query="Q2?", file_ids=[file.id])
...
metadata = client.files.metadata(file_id=file.id)
...
for doc in documents:
    ...
    db.save(file.id)  # Store in database
