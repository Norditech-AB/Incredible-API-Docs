...
# Request 1:
requests.post(
    "https://api.incredible.one/v1/agentic-model/chat/completions",
    headers={"Authorization": "Bearer YOUR_API_KEY"},
    json={
        "messages": [
            {
...
// Request 1:
await fetch('https://api.incredible.one/v1/agentic-model/chat/completions', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json'
...
# Request 1:
curl -X POST "https://api.incredible.one/v1/agentic-model/chat/completions" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [
