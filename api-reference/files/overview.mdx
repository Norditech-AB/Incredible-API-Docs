...
Upload files **once** during setup or when users first add them. Store the `file_id` and reuse it across unlimited chat requests. Avoid re-uploading the same file repeatedly—this wastes time and resources.
</Warning>
...
console.log(answerResponse.answer);

// Use with Conversation endpoint - multi-turn discussion
const convResponse = await client.conversation({
  messages: [
    { role: 'user', content: 'What are the key findings?', file_ids: [file_id] }
  ]
});
console.log(convResponse.response);
...
# Use with Answer endpoint - quick Q&A
...
curl -X POST "https://api.incredible.one/v1/conversation" \
  ...
  -d '{
    "messages": [
      {"role": "user", "content": "What are the key findings?", "file_ids": ["550e8400-..."]}
    ]
  }'

# Use with Agent endpoint - complex tasks
curl -X POST "https://api.incredible.one/v1/agent" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [
      {"role": "user", "content": "Analyze and create a chart", "file_ids": ["550e8400-..."]}
    ],
    "tools": [...]
  }'
```
